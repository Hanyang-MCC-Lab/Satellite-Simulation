...
from tkinter import *
=======
import pyautogui # 해상도 적응모듈
import keyboard # 키보드 입력 감지모듈
>>>>>>> Stashed changes
import math
...
class Orbit:
    ...
    def __init__(self, index, inclination, altitude, lon_of_ascending, repeat):
        ...
        for idx in range(CONST_SAT_NUM):
            sat = Satellite(self, idx, inclination, altitude, idx * CONST_SAT_ROT +repeat)
...
def deploy(inc, axis, repeat): #궤도생성
    if int(math.degrees(inc)) is 90: #Walker Star일 때
        for i in range(int(CONST_ORBIT_NUM / 2)):  # 궤도생성
            orbits.append(Orbit(i, inc, axis, CONST_ORBIT_ROT * i, repeat))
    else:
        for i in range(CONST_ORBIT_NUM): #Walker Delta일 때
            orbits.append(Orbit(i, inc, axis, CONST_ORBIT_ROT * i, repeat))
>>>>>>> Stashed changes
...
repeat = 0
while 1: # 복수궤도 입력
    inclination = math.radians(float(input("Please input Orbit Inclination radian.\n Orbit Inclination : ")))  # 궤도경사 입력
    altitude = int(input("Please input Satellite Altitude.\n Altitude : ")) # 궤도 반지름 입력
    while keyboard.is_pressed("q")==False: # 키보드 인터럽트를 통해 신규입력을 받기 전 지속하여 각속도 add
        deploy(inclination, altitude, repeat)  # 위성배치
        repeat += 0.5
