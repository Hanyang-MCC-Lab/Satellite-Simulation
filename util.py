...
def set_routing_simulation_result(ALGORITHM):
    folder_path = f'./SimulationResult/{ALGORITHM}/'
    ...
    with open(filename, 'w', encoding='utf-8', newline='') as file:
        ...
        wr.writerow(title)
        file.close()
def write_routing_simulation_result(ALGORITHM, data, gamma):
    filename = f'./SimulationResult/{ALGORITHM}/{ALGORITHM}result.csv'
    with open(filename, 'a', encoding='utf-8', newline='') as file:
        ...
        cnt_of_loglen_10000 = 0
        for log in data:
            if len(log.path) >= 200:
                cnt_of_loglen_10000 += 1
                continue
            ...
            # wr.writerow(row_data)
        avg_hops = sum_of_hops / (num_of_packets - cnt_of_loglen_10000)
        avg_delay = sum_of_delay / (num_of_packets - cnt_of_loglen_10000)
        avg_fails = sum_of_fails / (num_of_packets - cnt_of_loglen_10000)
        avg_overhead_msg = sum_of_overhead_msg / (num_of_packets - cnt_of_loglen_10000)
        wr.writerow([gamma, (num_of_packets - cnt_of_loglen_10000), avg_hops, avg_delay, sum_of_fails, avg_fails, sum_of_overhead_msg, avg_overhead_msg])
        file.close()
